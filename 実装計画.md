...
- [ ] **PoC-1: Marp CLI → Google Slides 変換品質検証**
  - [ ] marp-cliインストール、2カラム/図解サンプルMarkdown作成
  - [ ] `marp --pptx` でPPTX生成
  - [ ] Google DriveでSlides変換、目視確認（列幅±5%、フォント適用）
  - [ ] 結果を `poc-results.md` に記録
- [ ] **PoC-2: `drive.file` スコープ + Picker検証**
  - [ ] GCP OAuthクライアントID・Picker APIキー取得
  - [ ] 最小HTMLでPKCEフロー実装
  - [ ] Pickerフォルダ選択 → Driveアップロード+Slides変換の動作確認
  - [ ] 結果を記録
- [ ] **PoC-3: Workerコンテナ計測**
  - [ ] Dockerfile作成（node:20-slim + Chromium + marp-cli + Noto Sans JP）
  - [ ] 変換時間計測（5スライド、PPTX/PDF）
  - [ ] Cloud Runコールドスタート計測（min-instances=0 vs 1）
  - [ ] 結果を記録
- [ ] **Go/No-Go判断**
...
- [ ] pnpmワークスペース初期化（`packages/frontend`, `packages/backend`, `packages/shared`）
- [ ] ルート `package.json` にワークスペーススクリプト定義
- [ ] TypeScript共通設定（`tsconfig.base.json` strict: true）
- [ ] 各パッケージの `tsconfig.json`
- [ ] `packages/shared/types/` に共通型定義（`job.ts`, `api.ts`）
- [ ] ESLint flat config + Prettier設定
- [ ] `lint-staged` + `husky` pre-commitフック
- [ ] `.gitignore`, `.env.example`
- [ ] GitHub Actions CI（`.github/workflows/ci.yml` — lint, type-check, test）
- [ ] `docker/worker/Dockerfile`（Chromium + marp-cli + Noto Sans JP）
