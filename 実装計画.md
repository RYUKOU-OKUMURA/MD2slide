...
- [x] Next.js 14 (App Router) セットアップ + Tailwind CSS
- [x] レイアウトコンポーネント（`app/layout.tsx`）
- [x] CodeMirror 6エディタ統合（`components/editor/MarkdownEditor.tsx`）
  - [x] Markdown構文ハイライト + Marpディレクティブ対応
  - [x] Undo/Redo対応
- [x] Marpプレビューレンダラー（`lib/marp/renderer.ts` + `components/preview/SlidePreview.tsx`）
  - [x] 16:9固定、デバウンス300ms
  - [x] スライドページネーション（前/次）
- [x] エディタ+プレビュー分割レイアウト（リサイズ可能スプリットペイン）
- [x] ローカル保存（LocalStorage、自動保存3秒デバウンス、起動時復元）
- [x] テスト（エディタレンダリング、renderer単体、自動保存/復元）
...
- [x] Fastifyサーバー初期化（`src/server.ts`）+ CORS, ヘルスチェック
- [x] 環境変数管理（dotenv + zodバリデーション、`src/config.ts`）
- [x] `POST /api/export` エンドポイント（zodスキーマバリデーション、jobId発行）
- [x] `GET /api/jobs/:jobId/status` エンドポイント
- [x] ジョブキュー抽象化インターフェース（`src/queue/interface.ts`）
- [x] インメモリキュー実装（`src/queue/inMemoryQueue.ts`）
- [ ] PostgreSQLスキーマ定義（Drizzle ORM、`jobs`テーブル）+ マイグレーション
- [x] Marp CLIラッパー（`src/worker/marpConverter.ts` — PPTX/PDF生成、3分タイムアウト）
- [x] ジョブプロセッサ（`src/worker/processor.ts`）
- [x] CSS検証（`src/security/cssValidator.ts` — ホワイトリスト方式）
- [x] 画像URL検証（`src/security/imageUrlValidator.ts` — SSRF対策）
- [x] 一時ファイル管理（`src/worker/tempFileManager.ts` — 即時削除+30分バッチ削除）
- [x] PDFダウンロードエンドポイント（`GET /api/download/:jobId`）
- [x] テスト（API統合、marpConverter単体、セキュリティバリデータ単体）
...
- [x] Google OAuth PKCE実装（`lib/auth/googleAuth.ts` — verifier生成、認可URL、token交換）
- [x] トークン管理（`lib/auth/tokenManager.ts` — sessionStorage、自動リフレッシュ）
- [x] ログイン/ログアウトUI（`components/auth/LoginButton.tsx` + `hooks/useAuth.ts`）
- [x] Google Picker統合（`lib/google/pickerLoader.ts` + `components/google/FolderPicker.tsx`）
- [x] フォルダID保存（LocalStorage、初回表示+設定変更UI）
- [x] Drive APIクライアント（`src/google/driveClient.ts` — Resumable upload + mimeType変換）
- [x] トークン検証（`src/google/tokenValidator.ts` — 形式チェック、ログマスク）
- [x] Worker拡張: format="slides"時のDrive連携統合
- [x] テスト（OAuthフロー、Drive APIモック、Picker、トークンリフレッシュ）
