...
1. **フェーズ単位の段階実装**: `実装計画.md` のPhaseを必ず1つずつ順番に実装すること。1つのPhaseが完了するまで次のPhaseに進まない。
2. **自律的コードレビュー**: 各Phaseの実装完了後、自律的にコードレビューを行うこと。セキュリティ、設計整合性、コード品質を確認する。
3. **自律的テスト実行と修正**: 各Phase完了後、自律的にテストを実行し、致命的なエラーがなくなるまで修正を繰り返すこと。
4. **致命的なエラーの定義**: 以下に限定する。修正対象はこれらのみとし、軽微な警告やスタイルの問題は対象外。
   - 情報の漏洩リスク（トークン露出、機密データの非暗号化など）
   - 外部からの不正侵入リスク（SSRF、インジェクション、認証バイパスなど）
   - 機能が正常に動作しない（クラッシュ、主要機能の不具合、データ破損など）
